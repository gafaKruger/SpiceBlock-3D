...
public class ListaCones {
    ...
    public ListaCones(ArrayList<Cone> listaCones) {
        isAgrupado = false;
        centroX = 0;
        centroY = 0;
        centroZ = 0;
    ...
    public void defineFatores() {
        ...
        if (isAgrupado) {
            ...
            fatorEscalaZ = listaCones.get(0).getMatriz().getFatorEscalaZ();
            fatorTraslacaoX = listaCones.get(0).getMatriz().getFatorTraslacaoX();
            fatorTraslacaoY = listaCones.get(0).getMatriz().getFatorTraslacaoY();
            fatorTraslacaoZ = listaCones.get(0).getMatriz().getFatorTraslacaoZ();
            anguloRotacaoX = listaCones.get(0).getMatriz().getAnguloRotacaoX();
            anguloRotacaoY = listaCones.get(0).getMatriz().getAnguloRotacaoY();
            anguloRotacaoZ = listaCones.get(0).getMatriz().getAnguloRotacaoZ();
        }
    }
    ...
    public void transladarConesXY(int a, int b, int altura, int largura) {}
    
    public void transladarConesXZ(int a, int b, int altura, int largura) {}
    
    public void transladarConesZY(int a, int b, int altura, int largura) {}
    
    public void transladarConesXY(int a, int b) {}
    
    public void transladarConesXZ(int a, int b) {}
    
    public void transladarConesZY(int a, int b) {}
    ...
    */
    ...
    public ArrayList<Cone> getListaCones() {
        return listaCones;
    }

    public Cone getCone() {
        return listaCones.get(0);
    }

    public void setListaCones(ArrayList<Cone> lista) {
        this.listaCones = lista;
    }
    ...
    public int getTamanhoLista() {
        return listaCones.size();
    }

    public void setarAgrupamentoCones() {
        if (isAgrupado()) {
            for (int i = 0; i < listaCones.size(); i++) {
                listaCones.get(i).setAgrupado(true);
            }
        } else {
            listaCones.get(0).setAgrupado(false);
        }
    }
